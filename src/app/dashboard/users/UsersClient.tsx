  ...
  return (
    <div>
      ...
      <div className="relative group">
        {Icon && (
          <div className="absolute left-0 top-0 bottom-0 w-11 flex items-center justify-center pointer-events-none z-10">
            <Icon
              className="text-[var(--c-muted)] group-focus-within:text-violet-400 transition-colors"
              size={18}
            />
          </div>
        )}
        {children ?? (
          <input
            ...
            disabled={disabled}
            autoComplete="off"
            name={`saturn_uf__${label.replace(/\s+/g, "_").toLowerCase()}`}
      ...
      {maxLength !== undefined && value !== undefined && (
        <p className="text-right text-[10px] text-[var(--c-muted)] mt-0.5">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-16 h-16 rounded-2xl overflow-hidden bg-gradient-to-br from-violet-600 to-cyan-600 flex-shrink-0 border-2 border-[var(--c-border)]">
            {value ? (
              ...
              <div className="w-full h-full flex items-center justify-center">
                <MdPerson size={28} className="text-white/60" />
              </div>
            )}
            {uploading && (
          ...
          <div className="flex-1 space-y-1.5">
            ...
            >
              {uploading ? (
                <>
                  <MdRefresh className="animate-spin" size={15} /> {progress}%
                </>
              ) : (
                <>
...
function UserPreviewPanel({
  ...
  return (
    ...
    >
      <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--c-muted)] mb-4 text-center">
        Preview
      </p>
      ...
      <div className="flex flex-col items-center gap-2 mb-4">
        ...
        <div className="text-center">
          <p className="font-bold text-[var(--c-text)] text-sm leading-tight truncate max-w-[130px]">
            {displayName}
          </p>
          <p className="text-[var(--c-muted)] text-xs font-mono mt-0.5 truncate max-w-[130px]">
            @{form.username || "username"}
          </p>
          <span
            className={`mt-1.5 inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${ROLE_COLORS[form.role] || ROLE_COLORS.user}`}
          >
            {form.role || "user"}
          </span>
        </div>
      ...
      <div className="space-y-2 flex-1">
        {[
          ...
          val ? (
            ...
            >
              <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--c-muted)] mb-0.5">
                {label}
              </p>
              <p className="text-[var(--c-text)] text-xs font-mono truncate">
                {val}
              </p>
            </div>
          ) : null,
      ...
      {/* Limits */}
      <div
        className="mt-3 rounded-xl p-2.5 text-[10px] space-y-1"
        style={{
          background: "var(--c-input-bg)",
          border: "1px solid var(--c-border)",
        }}
      >
        <p className="font-bold uppercase tracking-wider text-[var(--c-muted)] mb-1.5">
          Limits
        </p>
        {[
          ["Username", "3–20 chars"],
...
function AddUserModal({
  ...
  const handleSubmit = async () => {
    if (!form.username || !form.password || !form.email) {
      toastError("Username, password & email required");
      return;
    }
    if (form.username.length < 3) {
      toastError("Username min 3 chars");
      return;
    }
    if (form.password.length < 6) {
      toastError("Password min 6 chars");
      return;
    }
    setLoading(true);
    try {
      const res = await fetch("/api/users", {
        ...
        body: JSON.stringify({
          ...form,
          displayName:
            `${form.firstName} ${form.lastName}`.trim() || form.username,
        }),
      });
  ...
  return (
    <Modal open onClose={loading ? undefined : onClose}>
      <SavingOverlay
        ...
        submessage="Setting up & syncing to GitHub"
      />
      <div
      ...
      >
        ...
        >
          ...
          <div className="flex-1">
            <h2 className="font-orbitron text-sm font-bold text-[var(--c-text)]">
              Add New User
            </h2>
          ...
          {!loading && (
            <button
              onClick={onClose}
              className="text-[var(--c-muted)] hover:text-[var(--c-text)] p-1.5 rounded-lg transition-colors"
            >
              <MdClose size={18} />
            </button>
          )}
        ...
        {/* Footer */}
        <div
          className="flex gap-3 px-5 py-3.5 border-t flex-shrink-0"
          style={{
            borderColor: "var(--c-border)",
            background: "var(--c-surface)",
          }}
        >
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 btn-primary justify-center py-2.5 disabled:opacity-70"
          >
...
function EditUserModal({ target, onClose, onSaved }: EditModalProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toastError(err.message || "Error saving");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal open onClose={saving ? undefined : onClose}>
      <SavingOverlay
        visible={saving}
        message="Saving & Syncing…"
        submessage="Updating user & pushing to GitHub"
      />
      <div
        className="w-full rounded-2xl overflow-hidden shadow-2xl flex flex-col"
        style={{
      ...
      >
        ...
        >
          ...
          >
            {saving ? (
              <>
                <MdRefresh className="animate-spin" size={17} /> Saving…
              </>
            ) : (
              <>
                <MdSave size={17} /> Save Changes
              </>
            )}
          </button>
        </div>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            disabled={loading}
            className="text-[var(--c-muted)] hover:text-[var(--c-text)] p-1 ml-2"
          >
            <MdClose size={18} />
          </button>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? (
                <>
                  <MdRefresh className="animate-spin" size={16} /> Saving…
                </>
              ) : (
                <>
                  <MdSave size={16} /> Save Reason
                </>
              )}
            </button>
          </div>
    ...
    <Modal open onClose={loading ? undefined : onClose}>
      ...
      >
        ...
        >
          ...
          <h3 className="font-orbitron text-sm font-bold text-red-400 flex-1">
            Ban @{target.username}
          </h3>
          {!loading && (
            <button
              onClick={onClose}
              className="text-[var(--c-muted)] hover:text-[var(--c-text)] p-1"
            >
              <MdClose size={18} />
            </button>
          )}
        ...
        <div className="p-5">
          <p className="text-[var(--c-muted)] text-sm mb-4 font-nunito">
            They will see a banned screen on next login.
          </p>
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            ...
            autoComplete="off"
            name="saturn_ban__reason"
            className="saturn-input resize-none w-full"
            disabled={loading}
          />
        </div>
        <div className="flex gap-3 px-5 pb-5">
          ...
          >
            {loading ? (
              <>
                <MdRefresh className="animate-spin" size={16} /> Banning…
              </>
            ) : (
              <>
                <MdBlock size={16} /> Confirm Ban
              </>
            )}
          </button>
        </div>
...
export default function UsersClient({
  ...
  const [sort, setSort] = useSessionState<string>("users_sort", "role");
  ...
  const [editReasonTarget, setEditReasonTarget] = useState<PublicUser | null>(
    null,
  );
  ...
  const [bulkQRResult, setBulkQRResult] = useState<{
    generated: number;
    failed: number;
  } | null>(null);
  ...
  const doBulkQR = async () => {
    ...
    try {
      const res = await fetch("/api/users/qr-bulk-generate", {
        method: "POST",
      });
      const d = await res.json();
      if (d.success) {
  ...
  const sorted = (() => {
    ...
    if (sort === "role") {
      arr.sort(
        (a, b) =>
          (roleHierarchy[a.role] || 100) - (roleHierarchy[b.role] || 100),
      );
    } else if (sort === "name") {
      arr.sort((a, b) => {
  ...
  // UI: Sorting select + icon, placed to the right of search input
  function SortMenu() {
    return (
      <div
        className="flex items-center gap-1 absolute right-0 top-0 bottom-0 px-1.5 z-20"
        style={{ height: "100%" }}
      >
        <select
          value={sort}
          onChange={(e) => setSort(e.target.value)}
          className="saturn-input pr-6 pl-3 py-1.5 h-8 text-xs rounded-xl border border-[var(--c-border)]"
          style={{
            minWidth: 120,
            background: "var(--c-input-bg)",
            color: "var(--c-muted)",
        ...
        >
          {(() => {
            const Icon =
              sortOptions.find((opt) => opt.value === sort)?.icon || MdSort;
            return <Icon size={17} />;
          })()}
        </span>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <SavingOverlay
        visible={!!actionLoading || !!deletingId}
        message={
          actionLoading ? `Processing: ${actionLoading}…` : "Deleting User…"
        }
      ...
      <div className="relative group" style={{ position: "relative" }}>
        ...
        <input
          ...
          autoComplete="off"
          name="saturn_users__search"
          className="saturn-input w-full pr-36 focus:outline-none"
          style={{ paddingLeft: "44px" }}
        />
        <SortMenu />
      </div>
      ...
      >
        <div className="overflow-x-auto sm:overflow-visible">
          <table className="w-full min-w-[580px] sm:min-w-full">
            <thead>
              <tr
                className="border-b"
                style={{ borderColor: "var(--c-border)" }}
              >
                {["User", "Email", "Role", "Joined", "Status", "Actions"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3.5 text-[var(--c-muted)] text-xs font-semibold uppercase tracking-wider"
                    >
                      {h}
                    </th>
                  ),
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={6}
                    className="text-center py-12 text-[var(--c-muted)]"
                  >
                    <MdPeople size={32} className="mx-auto mb-2 opacity-30" />
                    No users found
                  </td>
                </tr>
              ) : (
                sorted.map((u) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-[var(--c-text)] text-sm font-semibold truncate">
                          ...
                          {/* MOBILE EMAIL */}
                          <p className="text-[var(--c-muted)] text-xs font-mono truncate sm:hidden opacity-70 break-all">
                            {u.email}
                          </p>
                          ...
                          <div className="sm:hidden mt-1">
                            <span
                              className={`inline-block text-[10px] font-bold px-1.5 py-0.5 rounded-full border capitalize ${roleBadgeClass(u.role as any)}`}
                            >
                              {u.role}
                            </span>
                          </div>
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full border capitalize ${roleBadgeClass(u.role as any)}`}
                      >
                        {u.role}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {canManage(currentUser.role, u.role) ? (
                        ...
                        <MdPerson
                          size={15}
                          className="text-[var(--c-muted)] opacity-30"
                        />
                      )}
                    </td>
                  </tr>
      ...
      {banTarget && (
        <BanModal
          ...
          onConfirm={async (reason) => {
            await doAction(banTarget.id, "ban", { reason });
            setBanTarget(null);
          }}
        />
      )}
      ...
      <ConfirmModal
        ...
        isLoading={actionConfirmLoading}
      />
      ...
      {bulkQRResult && (
        <div
          className="fixed bottom-6 right-6 z-[9999] flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl"
          style={{
            background: "var(--c-surface)",
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--c-muted)" }}>
              {bulkQRResult.generated} berhasil
              {bulkQRResult.failed > 0 ? `, ${bulkQRResult.failed} gagal` : ""}
            </p>
          </div>
          <button
            onClick={() => setBulkQRResult(null)}
            className="ml-2"
            style={{ color: "var(--c-muted)" }}
          >
            ✕
              ...
              <div className="w-10 h-10 rounded-xl overflow-hidden bg-gradient-to-br from-violet-600 to-cyan-600 flex-shrink-0">
                {viewTarget.avatar ? (
                  <img
                    ...
                    alt={viewTarget.displayName}
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-white font-bold text-lg">
                    {(viewTarget.displayName ||
                      viewTarget.username)[0].toUpperCase()}
                  </div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-[var(--c-text)] truncate">
                  {viewTarget.displayName}
                </p>
                <p
                  ...
                  style={{ color: "var(--c-muted)" }}
                >
                  @{viewTarget.username} · {viewTarget.role}
                </p>
              </div>
              <button
                onClick={() => setViewTarget(null)}
                className="p-1.5 rounded-lg text-xl leading-none"
                style={{ color: "var(--c-muted)" }}
              >
                ✕
              </button>
