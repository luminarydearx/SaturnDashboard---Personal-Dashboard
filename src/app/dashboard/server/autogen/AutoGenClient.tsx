...
// ─── Constants ────────────────────────────────────────────────────────────────
// AutoGenerator-App is now a SEPARATE standalone repo
const AUTOGEN_URL =
  process.env.NEXT_PUBLIC_AUTOGEN_URL ||
  "https://auto-generator-app.vercel.app";
const AUTOGEN_GITHUB = `https://github.com/${process.env.NEXT_PUBLIC_AUTOGEN_GITHUB_OWNER || "luminarydearx"}/${process.env.NEXT_PUBLIC_AUTOGEN_GITHUB_REPO || "AutoGenerator-App"}`;
...
const BRANCH = "master";
// lockdown.json lives at root of AutoGenerator-App: public/lockdown.json
...
export default function AutoGenClient({ user: _user }: Props) {
    ...
    const tick = () => {
      ...
      if (schedule.lockdownEnabled && schedule.lockdownAt && !lockdownActive) {
        if (now >= new Date(schedule.lockdownAt).getTime()) {
          executeLockdown(
            schedule.lockdownReason,
            schedule.lockdownMediaUrl,
            true,
          );
        }
      }
  ...
  const pushToGitHub = async (
    ...
    try {
      const res = await fetch("/api/github/push", {
        ...
        body: JSON.stringify({
          owner: AUTOGEN_REPO_OWNER,
          repo: AUTOGEN_REPO_NAME,
          branch: BRANCH,
          files,
          message: msg,
        }),
      ...
      throw new Error(
        d.error || d.message || d.errors?.join(", ") || "Push gagal",
      );
    } catch (e: any) {
    ...
    try {
      ...
      if (mediaFile && !mediaUrl) {
        try {
          ...
          toastErr("Upload media gagal — lanjut tanpa media");
        }
      }
  ...
  const handleLockdown = () => {
    ...
    executeLockdown(lockdownReason);
  };
  const handleUnlock = () => {
    setShowUnlockConf(false);
    executeUnlock();
  };
  ...
  const restartServer = async () => {
    ...
    try {
      ...
      if (d.success) {
        success(
          `AutoGen redeploy dimulai! Selesai ~30 detik. ${d.deploymentId ? `(ID: ${d.deploymentId})` : ""}`,
        );
      } else {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg,rgba(var(--c-accent-rgb),.3),rgba(var(--c-accent2-rgb),.2))",
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {lockdownActive ? (
            <button
              onClick={() => setShowUnlockConf(true)}
              disabled={lockdownLoading || pushing}
            ...
            >
              <MdLockOpen size={15} />
              {lockdownLoading || pushing ? "…" : "Unlock"}
            </button>
          ) : (
            <button
            ...
            >
              <MdLock size={15} />
              {lockdownLoading || pushing ? "…" : "Lockdown"}
            </button>
          )}
          ...
          <div
            title={
              tokenSource === "env"
                ? `GITHUB_TOKEN: ${tokenPreview}`
                : "GITHUB_TOKEN tidak ditemukan di env!"
            }
          ...
          <button
            ...
            title="Redeploy AutoGen di Vercel"
          ...
          >
            <MdPowerSettingsNew
              size={14}
              className={restartLoading ? "animate-spin" : ""}
            />
      ...
      {tokenSource === "missing" && (
        ...
        >
          ...
          <p className="text-xs leading-relaxed text-[var(--c-muted)]">
            ...
            Tambahkan di Vercel Dashboard → SaturnDashboard → Settings →
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-5 items-start">
        ...
        >
          ...
          >
            <div
              className="flex items-center gap-1.5 flex-1 px-3 py-1.5 rounded-lg text-xs font-mono text-[var(--c-muted)]"
              style={{
                background: "var(--c-bg)",
                border: "1px solid var(--c-border)",
              }}
          ...
          {showIframe ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-5 py-20">
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg,rgba(var(--c-accent-rgb),.2),rgba(var(--c-accent2-rgb),.1))",
                  border: "1px solid rgba(var(--c-accent-rgb),.2)",
                }}
              >
              ...
              <div className="text-center">
                <p className="font-orbitron text-lg font-bold text-[var(--c-text)] mb-2">
                  Live Preview
                </p>
                <p className="text-[var(--c-muted)] text-sm">
                  Preview website AutoGen secara realtime
                </p>
              </div>
              <button
                onClick={() => {
              ...
              <div>
                <label className="block text-[var(--c-muted)] text-xs font-semibold uppercase tracking-wider mb-2">
                  ...
                  <span className="normal-case font-normal">
                    (opsional — ditampilkan ke pengunjung)
                  </span>
                </label>
                <textarea
              ...
              <div>
                ...
                {mediaPreview || mediaUrl ? (
                  ...
                  >
                    ...
                    {mediaUrl && !mediaFile && (
                      <div
                        className="absolute bottom-0 left-0 right-0 px-3 py-1 text-[10px] font-mono truncate"
                        style={{
                  ...
                  >
                    ...
                    <span className="text-xs opacity-60">
                      JPG, PNG, GIF, MP4, WebM — maks 20MB
                    </span>
                  </button>
                )}
                <input
                ...
                {uploadingMedia && (
                  <div className="flex items-center gap-2 mt-2 text-xs text-[var(--c-muted)]">
                    <MdRefresh size={13} className="animate-spin" /> Mengupload
                    ke Cloudinary…
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[var(--c-text)] text-sm font-semibold">
                      Auto-Lockdown
                    </span>
                  </div>
                  {/* Toggle */}
                  <button
                    ...
                    style={{
                      background: schedule.lockdownEnabled
                        ? "var(--c-accent)"
                        : "var(--c-border)",
                    }}
                  >
                    <span
                      ...
                      style={{
                        left: schedule.lockdownEnabled ? "22px" : "2px",
                      }}
                    />
                  </button>
                </div>
                ...
                {schedule.lockdownEnabled && (
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSchedule((s) => ({
                            ...s,
                            lockdownAt: fromLocalDT(e.target.value),
                          }))
                        }
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={schedule.lockdownReason}
                        onChange={(e) =>
                          setSchedule((s) => ({
                            ...s,
                            lockdownReason: e.target.value,
                          }))
                        }
                    ...
                    <div>
                      <label className="block text-[var(--c-muted)] text-xs font-semibold uppercase tracking-wider mb-1.5">
                        URL Media{" "}
                        <span className="normal-case font-normal">
                          (opsional — Cloudinary URL)
                        </span>
                      </label>
                      <input
                        type="url"
                        value={schedule.lockdownMediaUrl}
                        onChange={(e) =>
                          setSchedule((s) => ({
                            ...s,
                            lockdownMediaUrl: e.target.value,
                          }))
                        }
                        placeholder="https://res.cloudinary.com/…"
                        className="saturn-input w-full focus:outline-none text-sm font-mono"
                        style={{ paddingLeft: 12 }}
                      />
                      <p className="text-[var(--c-muted)] text-xs mt-1">
                        Upload media dulu di panel Lockdown Control di atas,
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() =>
                      setSchedule((s) => ({
                        ...s,
                        unlockEnabled: !s.unlockEnabled,
                      }))
                    }
                    className="w-11 h-6 rounded-full transition-all relative flex-shrink-0"
                    style={{
                      background: schedule.unlockEnabled
                        ? "#22c55e"
                        : "var(--c-border)",
                    }}
                  >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSchedule((s) => ({
                            ...s,
                            unlockAt: fromLocalDT(e.target.value),
                          }))
                        }
                        className="saturn-input w-full focus:outline-none text-sm"
                        style={{ paddingLeft: 12 }}
          ...
          >
            <div className="p-6">
              ...
              <p className="text-sm text-[var(--c-muted)] text-center mb-4 font-nunito">
                Website AutoGen akan menampilkan halaman lockdown ke semua
                pengunjung. Push ke GitHub → Vercel auto-deploy.
              </p>
              {lockdownReason && (
                <div
              ...
              {mediaFile && !mediaUrl && (
                <div className="flex items-center gap-2 text-xs text-amber-400 mb-2">
                  <MdUpload size={14} /> Media akan diupload ke Cloudinary saat
                  proses
                </div>
              )}
            </div>
