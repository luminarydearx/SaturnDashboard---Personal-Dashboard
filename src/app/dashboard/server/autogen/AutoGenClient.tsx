...
export default function AutoGenClient({ user: _user }: Props) {
  ...
  // ─── On mount ─────────────────────────────────────────────────────────────
  useEffect(() => {
    // Load lockdown state from local DB (instant, always accurate after push)
    fetch("/api/autogen/lockdown-state", { cache: "no-store" })
      .then((r) => r.json())
      .then((d: LockdownJson & { error?: string }) => {
        if (!d || d.error) return;
        setLockdownData(d);
    ...
    // Load announce state from local DB
    fetch("/api/autogen/announce", { cache: "no-store" })
      .then((r) => r.json())
      .then((d: any) => {
        if (d?.active) {
  ...
  const executeLockdown = async (
    ...
    try {
      ...
      if (ok) {
        ...
        // Persist lockdown state to DB so reload shows correct state
        fetch("/api/autogen/lockdown-state", {
  ...
  const executeUnlock = async (fromSchedule = false) => {
    ...
    try {
      ...
      if (ok) {
        ...
        // Clear persisted lockdown state in DB
        const cleared = { active: false, reason: "", timestamp: new Date().toISOString() };
        fetch("/api/autogen/lockdown-state", {
          method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const saveAnnounce = async (active: boolean) => {
    ...
    try {
      const res = await fetch("/api/autogen/announce", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ active, message: aMsg, type: aType, link: aLink, linkText: aLinkTxt, branch }),
      });
      const d = await res.json();
      if (!d.success) throw new Error(d.error || "Gagal");
      setAActive(active);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--c-border)", background: "var(--c-bg)" }}>
        ...
        <div className="px-3 py-1.5 text-[10px] truncate"
          ...
          {env === "production" ? "⚠️ Live users — push ke branch master" : "✅ Safe test — push ke branch staging"}
        </div>
      </div>
