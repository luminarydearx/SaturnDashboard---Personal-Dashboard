      ...
      >
        ...
        <motion.form onSubmit={login} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <motion.div animate={{ scale: focus === 'pw' ? 1.01 : 1 }} transition={{ duration: 0.15 }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: focus === 'pw' ? '#a78bfa' : 'rgba(255,255,255,.3)', transition: 'color .2s' }}>
                <MdLock size={18} />
              </div>
              <input type={showPw ? 'text' : 'password'} value={lp} onChange={e => setLp(e.target.value)} autoComplete="current-password"
                onFocus={() => setFocus('pw')} onBlur={() => setFocus(null)}
                style={{
                  width: '100%', paddingLeft: 44, paddingRight: 48, paddingTop: 14, paddingBottom: 14,
                  background: focus === 'pw' ? 'rgba(124,58,237,.08)' : 'rgba(255,255,255,.04)',
                  border: focus === 'pw' ? '1px solid rgba(124,58,237,.5)' : '1px solid rgba(255,255,255,.08)',
                  borderRadius: 14, color: '#fff', fontSize: 15, outline: 'none',
                  transition: 'all .2s', boxSizing: 'border-box',
                  boxShadow: focus === 'pw' ? '0 0 20px rgba(124,58,237,.15)' : 'none',
                }} />
              <button type="button" onClick={() => setShowPw(p => !p)}
                style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'rgba(255,255,255,.35)', cursor: 'pointer', padding: 4 }}>
