...
export async function GET() {
  ...
  // ── Vercel fix: convert Node Buffer → Uint8Array (valid BodyInit) ─────
  const body = new Uint8Array(zipBuf.buffer, zipBuf.byteOffset, zipBuf.byteLength);

  // Register in backup list
  addBackupEntry({
    id: uuidv4(), name: fname, createdAt: new Date().toISOString(),
    size: zipBuf.length, type: 'local', createdBy: user.username,
  });
...
export async function POST(req: NextRequest) {
  ...
  const result = await pushToGithub({
    ...
    repo:  repoName,
    files: [
      { path: `backups/saturn-backup-${ts}.zip`, content: zipBuf.toString('base64') },
      { path: 'backups/latest.json',             content: JSON.stringify({ lastBackup: new Date().toISOString(), file: `saturn-backup-${ts}.zip` }, null, 2) },
    ],
  ...
  if (result.success) {
    addBackupEntry({
      id: uuidv4(), name: fname, createdAt: new Date().toISOString(),
      size: zipBuf.length, type: 'github', createdBy: user.username,
    });
    return NextResponse.json({ success: true, file: `backups/${fname}` });
  } else {
    return NextResponse.json({ success: false, error: result.errors.join(', ') });
  }
}
