...
export async function POST(req: NextRequest) {
  try {
    ...
    // Return the JWT token to the client — they store it in sessionStorage
    // and send it back when verifying the code
    return NextResponse.json({ success: true, token, message: 'Kode dikirim ke ' + lower });
  } catch (e: any) {
    console.error('send-verification error:', e);
    return NextResponse.json(
      { error: 'Gagal mengirim email. Periksa alamat email dan coba lagi.' },
      { status: 500 }
    );
  }
}
