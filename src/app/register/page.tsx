    ...
    <div style={{ minHeight: '100vh', background: '#04040d', display: 'flex', alignItems: 'stretch', overflow: 'hidden', position: 'relative' }}>
      ...
      <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: [0.16,1,0.3,1] }}
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            <motion.form key="s2" onSubmit={step2Next} initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:20 }} style={{ display:'flex', flexDirection:'column', gap:16 }}>
              <div>
                <label style={LB}>Username *</label>
                <div style={{ position:'relative' }}>
                  <div style={IC('un')}><MdPerson size={17} /></div>
                  <input value={reg.username} onChange={e=>sr('username')(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g,''))} autoComplete="username" onFocus={()=>setFocus('un')} onBlur={()=>setFocus(null)} style={IS('un')} placeholder="username_unik" />
                </div>
                {reg.username && reg.username.length < 3 && <p style={{ color:'#f87171', fontSize:11, margin:'6px 0 0', paddingLeft:4 }}>Min. 3 karakter</p>}
              </div>
              <div>
                <label style={LB}>Password *</label>
                <div style={{ position:'relative' }}>
                  <div style={IC('pw')}><MdLock size={17} /></div>
                  <input type={showPw?'text':'password'} value={reg.password} onChange={e=>sr('password')(e.target.value)} autoComplete="new-password" onFocus={()=>setFocus('pw')} onBlur={()=>setFocus(null)} style={{ ...IS('pw'), paddingRight:48 }} placeholder="min. 6 karakter" />
                  <button type="button" onClick={()=>setShowPw(p=>!p)} style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'rgba(255,255,255,.35)', cursor:'pointer', padding:4 }}>
                    {showPw ? <MdVisibilityOff size={18} /> : <MdVisibility size={18} />}
                  </button>
                </div>
                {reg.password.length > 0 && (
                  <div style={{ marginTop:8, display:'flex', gap:4 }}>
                    {[2,4,6,8].map(n=><div key={n} style={{ flex:1, height:3, borderRadius:2, background:reg.password.length>=n?(n<=4?'#f87171':n<=6?'#fbbf24':'#4ade80'):'rgba(255,255,255,.1)', transition:'background .3s' }} />)}
                  </div>
                )}
              </div>
              ...
              <div style={{ display:'flex', gap:10 }}>
                <motion.button type="button" onClick={()=>setStep(1)} whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }}
                  style={{ flex:1, padding:'15px', borderRadius:14, border:'1px solid rgba(255,255,255,.1)', cursor:'pointer', background:'rgba(255,255,255,.04)', color:'rgba(255,255,255,.6)', fontSize:14, fontWeight:600 }}>
                  ← Kembali
                </motion.button>
                <motion.button type="submit" disabled={loading} whileHover={{ scale:loading?1:1.02 }} whileTap={{ scale:loading?1:0.97 }}
                  style={{ flex:2, padding:'15px', borderRadius:14, border:'none', cursor:loading?'not-allowed':'pointer', background:loading?'rgba(124,58,237,.3)':'linear-gradient(135deg,#7c3aed,#0891b2)', color:'#fff', fontSize:14, fontWeight:700, boxShadow:loading?'none':'0 8px 25px rgba(124,58,237,.35)', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                  {loading ? <><div style={{ width:16, height:16, border:'2px solid rgba(255,255,255,.3)', borderTopColor:'#fff', borderRadius:'50%', animation:'spin .7s linear infinite' }} />Mengirim…</> : <><MdMarkEmailRead size={16} />Kirim Kode Verifikasi</>}
                </motion.button>
              </div>
            </motion.form>
          )}
