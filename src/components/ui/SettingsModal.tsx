...
import {
  ...
  MdContentCopy, MdNotes, MdRestore, MdQrCode2,
} from 'react-icons/md';
...
type Tab = 'profile' | 'notes' | 'theme' | 'artifact' | 'settings' | 'shortcuts' | 'system';
interface Snippet {
...
function ProfileTab({ user }: { user: PublicUser }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* ── QR Code ── */}
      <div className="rounded-2xl overflow-hidden" style={{ background: "var(--c-bg)", border: "1px solid var(--c-border)" }}>
        <div className="px-4 py-3 border-b flex items-center gap-2" style={{ borderColor: "var(--c-border)" }}>
          <MdQrCode2 size={15} style={{ color: "var(--c-accent)" }} />
          <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--c-muted)" }}>QR Code Login</p>
        </div>
...
// ── SystemTab (owner only) ─────────────────────────────────────────────────
function SystemTab({ user: _user }: { user: PublicUser }) {
  ...
  useEffect(() => {
    fetch("/api/system/info").then(r => r.json()).then(d => { if (d.success) setInfo(d); }).catch(() => {});
  }, []);

  const exportData = async () => {
    const r = await fetch("/api/backup", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "full", label: "settings-export" }) });
    const d = await r.json();
    if (d.success) { setExported(true); setTimeout(() => setExported(false), 3000); }
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl"
        ...
        <span className="text-red-400 text-xs font-bold">🔐 OWNER ONLY</span>
        <span className="text-xs" style={{ color: "var(--c-muted)" }}>Tab ini hanya terlihat oleh owner</span>
      </div>

      {/* System info */}
      <div>
        <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--c-muted)" }}>System Info</p>
        <div className="grid grid-cols-2 gap-2">
          {[
            { l: "Version",   v: "v6.3.0" },
            { l: "Next.js",   v: "16.2.0" },
            { l: "Role",      v: "Owner"   },
            { l: "Node ENV",  v: info?.nodeEnv || "production" },
          ].map(row => (
            <div key={row.l} className="px-3 py-2.5 rounded-xl"
              style={{ background: "var(--c-bg)", border: "1px solid var(--c-border)" }}>
              <p className="text-[10px] font-bold uppercase tracking-wider mb-0.5" style={{ color: "var(--c-muted)" }}>{row.l}</p>
              <p className="text-sm font-mono text-[var(--c-text)]">{row.v}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Actions */}
      <div>
        <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--c-muted)" }}>Quick Actions</p>
        <div className="flex flex-col gap-2">
          <button onClick={exportData}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all text-left"
            style={{ background: "var(--c-surface)", border: "1px solid var(--c-border)", color: exported ? "#4ade80" : "var(--c-text)" }}>
            <span className="text-lg">{exported ? "✅" : "📦"}</span>
            <div>
              <p className="font-bold">{exported ? "Export berhasil!" : "Export Full Backup"}</p>
              <p className="text-xs font-normal" style={{ color: "var(--c-muted)" }}>Buat backup semua data (users, notes, settings)</p>
            </div>
          </button>

          <button onClick={() => setConfirmClearLogs(true)} disabled={clearing}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all text-left"
            style={{ background: "rgba(239,68,68,.06)", border: "1px solid rgba(239,68,68,.2)", color: "#f87171" }}>
            <span className="text-lg">{clearing ? "⏳" : "🧹"}</span>
            <div>
              <p className="font-bold">Hapus Activity Logs</p>
              <p className="text-xs font-normal" style={{ color: "var(--c-muted)" }}>Bersihkan semua riwayat aktivitas</p>
            </div>
          </button>
        </div>
      </div>
      ...
      {/* Secret info */}
      <div>
        <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--c-muted)" }}>Draft Sync Secret</p>
        <div className="px-4 py-3 rounded-xl font-mono text-xs"
          ...
          <p className="mb-1">Saturn ENV: <span className="text-[var(--c-accent)]">AUTOGEN_DRAFT_SECRET</span></p>
          <p>AutoGen ENV: <span className="text-[var(--c-accent)]">VITE_DRAFT_SECRET</span></p>
          <p className="mt-2 text-[10px] opacity-60">Nilai harus sama di kedua project</p>
        </div>
      </div>
...
function ShortcutsTab({ user }: { user: PublicUser }) {
  const [shortcuts,    setShortcuts]    = useState<ShortcutMap>(DEFAULT_SHORTCUTS);
  const [editing,      setEditing]      = useState<keyof ShortcutMap | null>(null);
  const [livePreview,  setLivePreview]  = useState<string>("");   // what user is currently pressing
  const [saved,        setSaved]        = useState(false);

  useEffect(() => { setShortcuts(loadShortcuts(user.id)); }, [user.id]);

  // Listen while editing — show live preview on every keydown, only confirm on checkmark click
  useEffect(() => {
    if (!editing) return;
    const handler = (e: KeyboardEvent) => {
      e.preventDefault(); e.stopPropagation();
      if (e.key === "Escape") { setEditing(null); setLivePreview(""); return; }
      const sc = eventToShortcut(e);
      if (sc) setLivePreview(sc);          // show live combo as user presses keys
    };
    window.addEventListener("keydown", handler, true);
    return () => window.removeEventListener("keydown", handler, true);
  }, [editing]);

  const confirmEdit = (key: keyof ShortcutMap) => {
    if (livePreview) {
      setShortcuts(prev => ({ ...prev, [key]: livePreview }));
    }
    setEditing(null);
    setLivePreview("");
  };

  const cancelEdit = () => { setEditing(null); setLivePreview(""); };

  const handleSave = () => {
    saveShortcuts(shortcuts, user.id);
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
    // Dispatch event so Navbar updates its placeholder immediately
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="px-3 py-2.5 rounded-xl text-xs leading-relaxed"
        style={{ background: "var(--c-bg)", border: "1px solid var(--c-border)", color: "var(--c-muted)" }}>
        💡 Klik <strong style={{ color: "var(--c-text)" }}>Edit ✏️</strong>, tekan kombinasi tombol yang diinginkan (misal <kbd className="px-1 rounded font-mono text-[10px]" style={{ background: "var(--c-surface)", border: "1px solid var(--c-border)" }}>Ctrl+F</kbd>{" "}
        atau <kbd className="px-1 rounded font-mono text-[10px]" style={{ background: "var(--c-surface)", border: "1px solid var(--c-border)" }}>Ctrl+Shift+L</kbd>), lalu klik <strong style={{ color: "var(--c-text)" }}>✓ Konfirmasi</strong>. Tekan Esc untuk batal.
      </div>
      <div className="flex flex-col gap-2">
        {(Object.keys(shortcuts) as (keyof ShortcutMap)[]).map(key => {
          const isEditing = editing === key;
          return (
            <div key={key} className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all ${isEditing ? "ring-1 ring-[var(--c-accent)]" : ""}`}
              style={{ background: "var(--c-surface)", border: isEditing ? "1px solid rgba(var(--c-accent-rgb),.5)" : "1px solid var(--c-border)" }}>
              <div>
                <span className="text-sm font-semibold text-[var(--c-text)]">{SHORTCUT_LABELS[key]}</span>
                {isEditing && livePreview && (
                  <p className="text-[10px] mt-0.5" style={{ color: "var(--c-accent)" }}>
                    Terdeteksi: {displayShortcut(livePreview)}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-2">
                {isEditing ? (
                  <>
                    <span className="px-3 py-1.5 rounded-lg text-xs font-mono min-w-[80px] text-center"
                      style={{ background: "rgba(var(--c-accent-rgb),.12)", color: "var(--c-accent)", border: "1px solid rgba(var(--c-accent-rgb),.3)", animation: livePreview ? "none" : "pulse 1.5s infinite" }}>
                      {livePreview ? displayShortcut(livePreview) : "Tekan keys…"}
                    </span>
                    {/* Confirm button */}
                    <button onClick={() => confirmEdit(key)} disabled={!livePreview}
                      title="Konfirmasi shortcut ini"
                      className="w-8 h-8 rounded-lg flex items-center justify-center transition-all font-bold"
                      style={{ background: livePreview ? "rgba(34,197,94,.15)" : "var(--c-bg)", color: livePreview ? "#4ade80" : "var(--c-muted)", border: livePreview ? "1px solid rgba(34,197,94,.35)" : "1px solid var(--c-border)" }}>
                      <MdCheck size={14} />
                    </button>
                    {/* Cancel button */}
                    <button onClick={cancelEdit} title="Batal"
                  ...
                  <>
                    ...
                    <button onClick={() => { setEditing(key); setLivePreview(""); }}
                      title={`Edit shortcut: ${SHORTCUT_LABELS[key]}`}
      ...
      <div className="flex gap-3">
        ...
        <button onClick={() => { setShortcuts({ ...DEFAULT_SHORTCUTS }); setEditing(null); setLivePreview(""); }}
          title="Reset ke shortcut default"
...
export default function SettingsModal({ open, onClose, user }: Props) {
  ...
  const TABS: { id: Tab; label: string; icon: React.ElementType }[] = [
    ...
    ...(user.role === 'owner' ? [{ id: 'system' as Tab, label: 'System', icon: MdSettings }] : []),
  ];
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5">
                <AnimatePresence mode="wait">
                  <motion.div key={tab}
                    ...
                    {tab === 'shortcuts' && <ShortcutsTab user={user} />}
                    {tab === 'system' && user.role === 'owner' && <SystemTab user={user} />}
                  </motion.div>
                </AnimatePresence>
              </div>
            </motion.div>
